...
function getTotal(arrNumber) {
  // code di sini
  const angkaTerbesar = arrNumber[arrNumber.length - 1];
  let frekuensi = 0;
  ...
  return arrNumber.length
    ? `angka paling besar adalah ${angkaTerbesar} dan jumlah kemunculan sebanyak ${frekuensi} kali`
    : "";
}
...
function changeVocals(str) {
  ...
  return str.replace(/[aeiou]/gi, (match) => {
    const nextChar = String.fromCharCode(match.charCodeAt(0) + 1);
    return nextChar;
  });
}
...
function setLowerUpperCase(str) {
  ...
  for (let i = 0; i < arrLetter.length; i++) {
    if (arrLetter[i] === arrLetter[i].toUpperCase()) {
      arrLetter[i] = arrLetter[i].toLowerCase();
    } else {
...
function passwordGenerator(name) {
  ...
  const upperLowerCase = setLowerUpperCase(reverse);
  const removeSpace = removeSpaces(upperLowerCase);
  return removeSpace;
}
...
// ## Soal 3
// ***INI SOAL PAS GUA INTERVIEW DI PERUSAHAAN FIVE JACK (YG PUNYA ITEMKU.COM)
// DAN GUA GAGAL PERKARA MALAH PAKE LOOPING WKWKWKW***
// ```js
/*
...
Function akan me-return nilai total dari digit tersebut dengan menambahkan satu per satu angka dari digit paling depan
 ke paling belakang. 
...
function totalDigitRekursif(angka) {
  ...
  if (angkaString.length < 1) {
    ...
    let firstNumber = Number(angkaString[0]);
    return (firstNumber += totalDigitRekursif(angkaString.slice(1)));
  }
}
...
function kaliTerusRekursif(angka) {
  ...
  if (angka < 10) {
    ...
    let hasilKali = 1;
    for (const element of angkaString) {
      hasilKali *= element;
    }
